...
function PricingBlock({ order }: { order: Order }) {
  ...
  // If total_amount was never set (old orders), fall back to subtotal
  const displayTotal = totalAmount > 0 ? totalAmount : subtotal

  return (
    <div
      className="rounded-2xl border overflow-hidden"
      style={{
        borderColor: hasCoupon ? 'rgba(52,211,153,0.2)' : 'rgba(255,255,255,0.08)',
        background:  hasCoupon ? 'rgba(52,211,153,0.04)' : 'rgba(255,255,255,0.03)',
      }}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b"
        style={{ borderColor: hasCoupon ? 'rgba(52,211,153,0.12)' : 'rgba(255,255,255,0.06)' }}>
      ...
      <div className="px-4 py-3 space-y-2">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-white/60 text-xs font-bold">
            {basePrice > 0 ? `${basePrice.toLocaleString()} DZD` : '—'}
          </span>
        </div>

        {/* Quantity line — only if > 1 */}
        {quantity > 1 && (
          <div className="flex items-center justify-between">
            <span className="text-white/35 text-xs">
              Quantity
        ...
        {/* Subtotal — only show separately when there's a coupon */}
        {hasCoupon && (
          <div className="flex items-center justify-between pt-1 border-t border-white/6">
            <span className="text-white/35 text-xs">Subtotal</span>
            <span className="text-white/60 text-xs font-bold">
            ...
            <span className="flex items-center gap-1.5 text-xs font-bold" style={{ color: '#34d399' }}>
              ...
              <svg className="w-3 h-3 shrink-0" fill="none" viewBox="0 0 24 24"
                ...
                <path strokeLinecap="round" strokeLinejoin="round"
                  d="M7 7h.01M7 3h5.586a1 1 0 01.707.293l7.414 7.414a2 2 0 010
                     2.828l-5.657 5.657a2 2 0 01-2.828 0L4.808 12.05A2 2 0
                     014.515 11.343V5.757A2.757 2.757 0 017 3z" />
              </svg>
              <span className="font-mono tracking-widest">{order.coupon_code}</span>
            </span>
            {hasDiscount ? (
        ...
        <div className="h-px" style={{
          background: hasCoupon ? 'rgba(52,211,153,0.15)' : 'rgba(255,255,255,0.07)',
        }} />

        {/* Total */}
        <div className="flex items-center justify-between">
          <span className="text-white/60 text-xs font-black tracking-wider uppercase">
            Total
          </span>
      ...
      >
          ...
          {variantEntries.length > 0 && (
            <div className="bg-blue-500/8 border border-blue-500/20 rounded-2xl p-4 space-y-2">
              ...
              {variantEntries.map(([label, value]) => (
                <div key={label}
                  className="flex justify-between items-start py-1.5 border-b border-white/5 gap-3 last:border-0">
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-white/8 shrink-0">
          <button
            onClick={handleSave}
            disabled={saving}
            className="bg-purple-600 hover:bg-purple-500 disabled:opacity-40 text-white font-black
                       px-6 py-2.5 rounded-xl text-xs tracking-widest uppercase transition-all cursor-pointer"
            style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
          >
    ...
    <>
      ...
      <div className="space-y-2">
        ...
        {displayedOrders.map(o => {
          ...
          const totalAmount   = parseFloat(String(o.total_amount || '0'))
          const basePrice     = parseFloat(String(o.price || '0'))
          const displayTotal  = totalAmount > 0 ? totalAmount : basePrice * o.quantity
          ...
          return (
            <div
              key={o.id}
              className="bg-white/4 border border-white/8 rounded-2xl px-5 py-4 flex items-center
                         gap-4 hover:border-purple-500/20 transition-colors"
            >
              {/* Thumbnail */}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-0.5">
                  <p className="text-white/20 text-[10px]">{formattedDate} · {o.email}</p>
                  {/* Inline total */}
                  {displayTotal > 0 && (
                    <span
